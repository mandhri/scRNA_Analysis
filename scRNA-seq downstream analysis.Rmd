...
SingleCellExperiment::colLabels(umi_filtered) <- umi_filtered$cluster


if (!"logcounts" %in% assayNames(umi_filtered)) {
  umi_filtered <- scuttle::logNormCounts(umi_filtered)
}

# donor-aware if possible; otherwise fall back
blk <- droplevels(factor(umi_filtered$individual))
sm <- if (nlevels(blk) >= 2) {
  scran::scoreMarkers(umi_filtered, block = blk, assay.type = "logcounts")
} else {
  scran::scoreMarkers(umi_filtered, assay.type = "logcounts")
}
