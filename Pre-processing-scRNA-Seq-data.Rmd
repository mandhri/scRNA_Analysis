...
```{r load_data}
molecules <- read.table("/mnt/vol1/scRNA_Analysis/DATASETS/Datasets/tung/molecules.txt", sep = "\t")
anno <- read.table("/mnt/vol1/scRNA_Analysis/DATASETS/Datasets/tung/annotation.txt", sep = "\t", header = TRUE)

reads <- read.table("/mnt/vol1/scRNA_Analysis/DATASETS/Datasets/tung/reads.txt", header = TRUE, stringsAsFactors = FALSE)
...
# Cleaning the single cell experimentw
...
# STEp 8: Calculate Per-Cell and Per-Feature QC Metrics
...
```{r exprs-qc17}
# Add donor & replicate as factors (from your anno columns)
umi$donor     <- factor(umi$individual)
umi$replicate <- factor(umi$replicate)

# Quick check
summary(umi$donor); summary(umi$replicate)
saveRDS(umi, file = "/mnt/vol1/scRNA_Analysis/DATASETS/Datasets/tung/umi.rds")
```
