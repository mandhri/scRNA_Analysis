...
library(edgeR)
library(SingleR)
librarlibrary(scDblFinder)y(celldex)
...
umi_filtered$label <- pred$labels
scater::plotReducedDim(umi_mnn, "UMAP", colour_by = I(umi_filtered$label[colnames(umi_mnn)]))
...
```{r}
# keep clusters from your corrected embedding
umi_filtered$cluster <- colLabels(umi_mnn)[colnames(umi_filtered)]

pb <- aggregateAcrossCells(
  umi_filtered,
  ids = DataFrame(cluster = umi_filtered$cluster,
                  donor   = umi_filtered$individual,
                  batch   = umi_filtered$replicate)
)

y <- DGEList(counts(assay(pb, "counts"))); y <- calcNormFactors(y)
design <- model.matrix(~ 0 + pb$cluster + pb$donor + pb$batch)
colnames(design) <- make.names(colnames(design))
y <- estimateDisp(y, design); fit <- glmQLFit(y, design)

# Example: cluster 3 vs 1 (adjusted for donor+batch)
con <- makeContrasts(cluster.3 - cluster.1, levels = design)
de_c3_vs_c1 <- topTags(glmQLFTest(fit, contrast = con), n = Inf)$table
